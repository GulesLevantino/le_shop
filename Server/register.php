...
if ($result->num_rows > 0) {
    echo json_encode(["status" => "failure", "message" => "Пользователь с таким логином или email уже есть"]);
    $stmt->close();
    $conn->close();
    exit();
}
...
if ($stmt->execute()) {
    ...
    echo json_encode(["status" => "success", "message" => "Пользователь зарегистрирован", "user_id" => $user_id]);
} else {
